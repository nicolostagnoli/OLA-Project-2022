...
def step3(m):
    ...
    marked_rows = np.setdiff1d(rows, assigned) # rows having NO assignment
...
def hungarian_algorithm(matrix):
    ...
    while n_lines != max_length:
        # At atep 3 we need to find the minimum number of lines (rows+columns) that we have to
        # draw to cover all zeros
        lines = step3(m)
        n_lines = len(lines[0]) + len(lines[1])
        print(f'n_lines = {n_lines}, max_length = {max_length}')
...
#a = np.array([[25,73,52],[43,3,76],[89,68,33]])
#a = np.array([[102,120,152],[152,139,174],[118,146,260]])
